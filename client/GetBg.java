...
public class GetBg implements Runnable {
	...
	public GetBg(FtpClient ftpClient, String hostname, int nPort, String[] tokens, Path serverPath, Path path) throws Exception {
...
//			if (ftpClient.terminateGET(path.resolve(tokens[1]), serverPath.resolve(tokens[1]), terminateID)) {
				if (pcount > 1000) { // check on terminate
						pcount = 0; // reset count for pause terminate
						if(ftpClient.terminateGET(path.resolve(tokens[1]), serverPath.resolve(tokens[1]), terminateID)) {
							f.close();
							return;
						}
