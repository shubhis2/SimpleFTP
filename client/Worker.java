	...
	public void run() {
		...
		//		else if ((tokens[0].equalsIgnoreCase("get")||tokens[0].equalsIgnoreCase("put")||tokens[0].equalsIgnoreCase("delete")||tokens[0].equalsIgnoreCase("mkdir")) && tokens.length != 2)
				...
				*************************************/
					...
					if (tokens.length > 2) {
						if (tokens[2].equals("&")) {
								...
								(new Thread(new GetWorker(ftpClient, hostname, nPort, tokens, filePath, filePathClient))).start();
			...
			//			long fileSize1 = file.length();  // ??? in thread ???
							(new Thread(new PutWorker(ftpClient, hostname, nPort, tokens, filePath))).start();
